...
- (id)sendRequest:(NSString *)path requestType:(UAGithubRequestType)requestType responseType:(UAGithubResponseType)responseType withParameters:(id)params page:(NSInteger)page error:(NSError * __strong *)error
{
	...
	switch (requestType) 
    {
        ...
        case UAGithubRepositoryContentCreateRequest://added
...
- (void)createFileWithInfo:(NSDictionary *)infoDictionary withPath:(NSString *)path forUser:(NSString *)aUser inRepo:(NSString *)repoName success:(UAGithubEngineSuccessBlock)successBlock failure:(UAGithubEngineFailureBlock)failureBlock
{
    [self invoke:^(id self) {
        [self sendRequest:[NSString stringWithFormat:@"repos/%@/%@/contents/%@", aUser, repoName, path] requestType:UAGithubRepositoryContentCreateRequest responseType:UAGithubRepositoryContentCreateResponse withParameters:infoDictionary  error:nil];
    } success:successBlock failure:failureBlock];
}

- (void)fileWithInfo:(NSDictionary *)infoDictionary withPath:(NSString *)path forUser:(NSString *)aUser inRepo:(NSString *)repoName success:(UAGithubEngineSuccessBlock)successBlock failure:(UAGithubEngineFailureBlock)failureBlock;
{
    [self invoke:^(id self) {
        [self sendRequest:[NSString stringWithFormat:@"repos/%@/%@/contents/%@", aUser, repoName, path] requestType:UAGithubRepositoryContentGetRequest responseType:UAGithubRepositoryContentGetResponse withParameters:infoDictionary  error:nil];
    } success:successBlock failure:failureBlock];
}
...
- (void)branchesForRepository:(NSString *)repositoryPath user:(NSString *)aUser success:(UAGithubEngineSuccessBlock)successBlock failure:(UAGithubEngineFailureBlock)failureBlock
{
	[self invoke:^(id self){[self sendRequest:[NSString stringWithFormat:@"repos/%@/%@/branches", aUser, repositoryPath] requestType:UAGithubBranchesRequest responseType:UAGithubBranchesResponse error:nil];} success:successBlock failure:failureBlock];
}
